...
int main(int argc, char* argv[]) {
  ...
  if (args.matrixFilename == "") {
    ...
    }
    {
      ...
      Ablock->describe(*fancy, Teuchos::VERB_LOW);
      B->describe(*fancy, Teuchos::VERB_LOW);
    }
  ...
  // Initial diagnostics
  Teuchos::Array<MT> normx(args.numVecs);
  Teuchos::Array<MT> normb(args.numVecs);
  X->norm2(normx);
  B->norm2(normb);
  if (rank0) {
  ...
  for (int repeat = 0; repeat < args.numRepeats; ++repeat) {
    ...
    if(nranks == 1) {
      ...
      auto Xl = X->getLocalViewDevice(Tpetra::Access::ReadOnly);
      auto Rh = Kokkos::create_mirror_view(Rl);
      auto Bh = Kokkos::create_mirror_view(Bl);
      auto Xh = Kokkos::create_mirror_view(Xl);
