...
//   RCP<const Tpetra::Map<> > map = A->getDomainMap();
   ...
   std::string Label ="QR factor time for HH level 1 ";
   std::string Label1 ="QR factor time MvDot operations in HH level 1";
   std::string Label2 ="QR factor time MvTransMv operations in HH level 1";
   std::string Label3 ="QR factor time MvAddMv operations in HH level 1";
   std::string Label4 ="QR factor time TRMV operations in HH level 1";
   std::string Label5 ="QR factor time GetLocalViewHost operations in HH level 1";
   std::string Label6 ="QR factor time Broadcasting operations in HH level 1";
   ...
   Teuchos::RCP<Teuchos::Time> timerIRSolve5_ = Teuchos::TimeMonitor::getNewCounter(Label5);
   Teuchos::RCP<Teuchos::Time> timerIRSolve6_ = Teuchos::TimeMonitor::getNewCounter(Label6);
#endif
   ...
   for( j=0; j<n; j++){
	 ...
	 { //scope guard for timer
         #ifdef BELOS_TEUCHOS_TIME_MONITOR
            Teuchos::TimeMonitor slvtimer(*timerIRSolve6_);
         #endif
	 ...
	 { //scope guard for timer
         #ifdef BELOS_TEUCHOS_TIME_MONITOR
            Teuchos::TimeMonitor slvtimer(*timerIRSolve5_);
         #endif
	 {
         if( my_rank == 0 ){
            {
