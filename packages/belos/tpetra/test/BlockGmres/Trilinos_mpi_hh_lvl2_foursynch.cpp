...
//   printf("\nsp=%3d, ep=%3d, rank=%3d\n",startingp,endingp,my_rank);
   ...
   for( j=0; j<n; j++){
      ...
      // The bulk of the HH algorithm
         ...
         // Combining the computation of V_{1:j-1}^T v_j
         MVT::MvTransMv( (+1.0e+00), *A_j, *a_j, *work );                 // Three AllReduce 

         // Finishing constructing T_{1:j-1,j}  
         ...
         RCP<MV> Broadcast_block = rcp( new MV(topblockmap,j) );
         Broadcast_block->doImport (*TopA_block, importer, Tpetra::INSERT);  // Two Broadcast
         auto b = Broadcast_block->getLocalViewHost();
