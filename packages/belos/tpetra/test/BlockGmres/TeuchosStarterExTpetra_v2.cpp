...
//   std::string filename("test1.hb");
   ...
   Teuchos::RCP<Teuchos::SerialDenseMatrix<int,ScalarType> > wwork; 
   ...
   if (wwork == Teuchos::null) {
     wwork = Teuchos::rcp( new Teuchos::SerialDenseMatrix<int,ScalarType>(1,1) );
   }  
...
//   printf("\n%3.2f\n", tmp);
   ...
   for( j=1; j<n; j++){
      ...
      wwork = Teuchos::rcp( new Teuchos::SerialDenseMatrix<int,ScalarType>(j,1) );

      A_j = *(AA->subViewNonConst(index_prev1));
      a_j = *(AA->subViewNonConst(index_prev2));
      q_j = *(QQ->subViewNonConst(index_prev2));
      
      MVT::MvTransMv( (+1.0e+00), A_j, a_j, *wwork );
      blas.TRMV( Teuchos::UPPER_TRI, Teuchos::TRANS, Teuchos::NON_UNIT_DIAG, j, &(*TT)(0,0), ldt, &(*wwork)(0,0), 1 ); // SerialDense Ops
      MVT::MvTimesMatAddMv( (-1.0e+00), A_j, *wwork, (+1.0e+00), a_j );      

      // Old Step 1: Project and Update
      ...
      MVT::MvTransMv( (+1.0e+00), A_j, q_j, *wwork );
      blas.TRMV( Teuchos::UPPER_TRI, Teuchos::NO_TRANS, Teuchos::NON_UNIT_DIAG, j, &(*TT)(0,0), ldt, &(*wwork)(0,0), 1 ); // SerialDense Ops
      MVT::MvTimesMatAddMv( (-1.0e+00), A_j, *wwork, (+1.0e+00), q_j );    
 
      // Old Step 3: Construct Q
      (*Q)(j,j) = 1.0e+00 - (*T)(j,j);
      ...
      // Step 4 for MV
      MVT::MvTransMv( (+1.0e+00), A_j, a_j, *wwork );
      for(i=0;i<j;i++) (*TT)(i,j) = (*wwork)(i,0);
      blas.SCAL( 1, -(*T)(j,j), &(*T)(0,j), 1 );
      blas.TRMV( Teuchos::UPPER_TRI, Teuchos::NO_TRANS, Teuchos::NON_UNIT_DIAG, 1, &(*TT)(0,0), ldt, &(*T)(0,j), 1 );

      // Old Step 4: Construct T
