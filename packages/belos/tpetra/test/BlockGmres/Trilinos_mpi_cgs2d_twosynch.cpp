...
//   RCP<const Tpetra::Map<> > map = A->getDomainMap();
   ...
   Teuchos::RCP<Teuchos::SerialDenseMatrix<int,ScalarType> > work1; 
   ...
   if (work1 == Teuchos::null) {
     work1 = Teuchos::rcp( new Teuchos::SerialDenseMatrix<int,ScalarType> );
   }  
   ...
   for( j=0; j<n; j++){
      ...
      if( j == 0 ){
         ...
         Teuchos::Range1D index_prev3(j,j);

         // I need two work spaces, I do not know how to only use one column on a 2 column serialdensematrix
         ...
         // Getting the delayed normalization into diagonal of R
         (*R)(j-1,j-1) = sqrt( (*work)(j-1,0) );

         // Catching up delayed norm
         (*work)(j-1,1) = (*work)(j-1,1) / (*R)(j-1,j-1);
         for(i=0;i<j;i++) (*work1)(i,0) = (*work)(i,1); 
         for(i=0;i<j;i++) (*R)(i,j) = (*work)(i,1);
         MVT::MvScale( *q_j, ( 1 / (*R)(j-1,j-1) ) );

         q_jnew = MVT::CloneViewNonConst( *Q, index_prev3 );
         MVT::MvScale( *q_jnew, (*R)(j-1,j-1) ); // undoing the scale since the above hits j-1 and j column

         MVT::MvTimesMatAddMv( (-1.0e+00), *Q_j, *work1, (+1.0e+00), *q_jnew );  

         MVT::MvTransMv( (+1.0e+00), *Q_j, *q_jnew, *work1 );        // Two AllReduce
         MVT::MvTimesMatAddMv( (-1.0e+00), *Q_j, *work1, (+1.0e+00), *q_jnew );  
