...
//   for(i=0;i<mloc;i++){ printf("%2d, ", ( startingp + i ) ); } printf("\n"); 
   ...
   size_t local_m, start;
   j = 3;
   if (startingp >= j) {
       // full
       local_m = mloc;
       offset = 0;
   } else if (startingp+mloc < j) {
       // empty
       local_m = 0;
       offset = mloc-1;
   } else {
...
//   RCP<const Tpetra::Map<int,long long,Node> >  submap = createContigMapWithNode<int,long long,Node>(INVALID, local_m, comm);
   RCP<const Tpetra::Map<ScalarType> >  submap;
   submap = Tpetra::createContigMapWithNode<ScalarType>(m, local_m, comm);
//   RCP<MV> A1 = AA->offsetViewNonConst(submap, offset);
 
   MVT::MvPrint( *AA, std::cout );
//   MV A_ij = *(AA->offsetViewNonConst(submap, j));
//   MVT::MvPrint( A1, std::cout );
