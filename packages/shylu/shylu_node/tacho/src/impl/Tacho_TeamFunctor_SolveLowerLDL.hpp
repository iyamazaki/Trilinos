...
template <typename SupernodeInfoType> struct TeamFunctor_SolveLowerLDL {
public:
...
public:
  ...
  KOKKOS_INLINE_FUNCTION
  TeamFunctor_SolveLowerLDL(const supernode_info_type &info, const ordinal_type_array &compute_mode,
  ...
  KOKKOS_INLINE_FUNCTION void operator()(const SolveTag<Var> &, const MemberType &member) const {
    ...
    if (mode == -1) {
      Kokkos::printf("Error: TeamFunctorSolveLowerLDL::SolveTag, computing mode is not determined\n");
    } else {
      // skip
    }
  }
