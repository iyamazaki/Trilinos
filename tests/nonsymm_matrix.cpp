...
struct CommandLineOptions {
  ...
  int  nx2 {-1};
  int  nx3 {-1};
  std::string GaleriName2D {"Laplace2D"};
  std::string GaleriName3D {"Laplace3D"};
  bool useDiagMat {true};
...
TEUCHOS_STATIC_SETUP()
{
  ...
  clp.setOption ("tsqr", &commandLineOptions.tsqrType,
                 "Name of the TSQR procedure");
  ...
  clp.setOption ("sketchDom", &commandLineOptions.sketchDom,
                 "Sketch domain; only applies to algorithms that take it.");
  clp.setOption ("useTwoStage", "noTwoStage", &commandLineOptions.useTwoStage,
                 "Whether to use two-stage");
  clp.setOption ("splitCGS", "noSplitCGS", &commandLineOptions.splitCGS,
                 "Whether to split CGS");
  clp.setOption ("useRandomQR", "noRandomQR", &commandLineOptions.useRandomQR,
                 "Whether to use RandomQR");
  clp.setOption ("useRandomCGS2", "noRandomCGS2", &commandLineOptions.useRandomCGS2,
                 "Whether to use RandomCGS2");
  ...
  clp.setOption ("useSVQR", "noSVQR", &commandLineOptions.useSVQR,
                 "Whether to use SVQR");
  clp.setOption ("useSVQR2", "noSVQR2", &commandLineOptions.useSVQR2,
                 "Whether to use SVQR2");
  clp.setOption ("useTSQR_1st_step", "noTSQR_1st_step", &commandLineOptions.useTSQR_1st_step,
                 "Whether to use CholQR2");
  ...
  clp.setOption ("galeriMatrixName2D", &commandLineOptions.GaleriName2D, "Name of 2D Galeri Matrix");
  clp.setOption ("galeriMatrixName3D", &commandLineOptions.GaleriName3D, "Name of 3D Galeri Matrix");
  ...
  clp.setOption ("rowmapFilename", &commandLineOptions.rowmapFilename, "Name of Rowmap file");
  ...
  clp.setOption ("subdomSolverType", &commandLineOptions.subdomSolverType,
                 "Which subdomain solver to use");
  clp.setOption ("Equil", "noEquil", &commandLineOptions.useEquil, "Whether to use Equil ");
  //
  ...
  //
  clp.setOption ("dampFactor", &commandLineOptions.dampFactor,
                 "Damp factor for relaxation");
  clp.setOption ("innerDampFactor", &commandLineOptions.innerDampFactor,
                 "Damp factor for inner relaxation");
  clp.setOption ("numRelaxationSweeps", &commandLineOptions.numRelaxationSweeps,
                 "Num relaxation sweeps");
  clp.setOption ("numOuterRelaxationSweeps", &commandLineOptions.numOuterRelaxationSweeps,
                 "Num outer relaxation sweeps");
  ...
  clp.setOption ("useCompactForm", "noCompactForm", &commandLineOptions.useCompactForm,
  ...
  //
  clp.setOption ("subdomainSolver", &commandLineOptions.subdomainSolver, "Name of subdomain solver");  
  //
...
readExaWind (std::string matfile, int nfiles, Teuchos::FancyOStream& out)
{
  ...
  auto origCrsMatrix = rcp (new crs_matrix_type (origRowMap, rowCounts));
  for (LO lclRow = 0; lclRow < origLclNumRows; ++lclRow) {
...
// nonsymmetric.
Teuchos::RCP<Tpetra::CrsMatrix<double> >
readSparseFile_withRowMap (std::string filename,
                           ...
                           const bool debug=false)
{
  ...
  LO localNnz = 0;
  {
    ...
    if ((fh = fopen(filename.c_str(), "r")) == NULL) {
      ...
      std::cout << commRank << ": reading:" << filename << std::endl;
    }
    // get first/last row and column indexes
    fscanf(fh, "%d %d %d\n", &nrow, &ncol, &nnz);
    if (commRank == 0) {
    ...
    // push the entry to local cooMatrix, and read the next entry
    while (ret != EOF) {
      //if (value != ZERO && rowMap->isNodeGlobalElement (irow))
      if (rowMap->isNodeGlobalElement (irow))
      {
        localNnz ++;
        cooMatrix.sumIntoGlobalValue (irow, icol, value);
      }
  ...
  if (commRank == 0) {
    printf( " > done reading file (local nnz = %d)\n",localNnz ); fflush(stdout);
  }
  ...
  // Count number of nonzeros per row
  const LO origLclNumRows = origRowMap->getLocalNumElements ();
  //std::cout << commRank << " : rowOffsets.size=" << rowOffsets.size() << " vs. " << origLclNumRows 
  //          << (rowOffsets.size() != 1+origLclNumRows ? " wraning" : "") << std::endl;
  Teuchos::Array<size_t> rowCountsArray (origLclNumRows);
  Teuchos::ArrayView<size_t> rowCounts = rowCountsArray (0, origLclNumRows);
  for (LO lclRow = 0; lclRow < origLclNumRows; ++lclRow) {
    rowCounts[lclRow] = rowOffsets[lclRow+1] - rowOffsets[lclRow];
  }
...
createDiagMatrix (const double diagAlpha, const double diagShift, const double diagPower, const Teuchos::RCP<const Tpetra::Map<> >& rowMap)
{
  ...
  const LO lclNumRows = rowMap.is_null () ? LO (0) :
    LO (rowMap->getLocalNumElements ());
  auto A = rcp (new crs_matrix_type (rowMap, 1));
            ...
            const bool verbose)
{
  ...
  RCP< Teuchos::Time > ioTimer = time_monitor_type::getNewCounter ("Matrix Create");
  {
    ...
    if (myRank == 0)
      std::cout << std::endl << " >> Create matrix << " << std::endl;
    if (commandLineOptions.matrixFilename != "") {
      if (commandLineOptions.rowmapFilename != "") {
        if (myRank == 0)
          myOut << "rowmap read from " << commandLineOptions.rowmapFilename << endl;
        auto rowMap = reader_type::readMapFile (commandLineOptions.rowmapFilename, comm);
        RCP<const map_type> colMap;

        if (myRank == 0)
          myOut << "matrix read from " << commandLineOptions.matrixFilename << endl;
        bool callFillComplete=true;
        bool tolerant=true; // not checking index bounds (snice file may be in global index)
        bool debug=false;
        ///A = reader_type::readSparseFile (commandLineOptions.matrixFilename, rowMap, colMap, rowMap, rowMap, callFillComplete, tolerant, debug);
        A = readSparseFile_withRowMap (commandLineOptions.matrixFilename, rowMap, out, callFillComplete, tolerant, debug);
      } else {
      ...
      if (commandLineOptions.GaleriName3D == "Elasticity3D") {
        GO mx = 1;
    ...
    try {
      ...
      RCP<Galeri_t> galeriProblem =
                    Galeri::Xpetra::BuildProblem<SC, LO, GO,
                                       ...
                                       (commandLineOptions.GaleriName3D, map, galeriList);
      A = galeriProblem->BuildMatrix();
    }
    catch (std::exception& e) {
      std::cout << "*** FAILED: Galeri::Xpetra::BuildProblem: "
          << e.what () << endl;
      success = false;
      return;
    }
      ...
      std::cout << "*** FAILED: Galeri::Xpetra::BuildProblem!!" << std::endl;
...
#endif
      if (myRank == 0) {
        std::cout << "3D " << commandLineOptions.GaleriName3D << " matrix with nx=" << commandLineOptions.nx3 << endl;
      }
    }
      ...
      galeriList.set("nx", nx);
      galeriList.set("ny", nx);
      if (commandLineOptions.GaleriName2D == "Elasticity2D") {
        nGlobalElements *= 2;
      }
      RCP<const map_type> map = rcp(new map_type(nGlobalElements, 0, comm));
      ...
      RCP<Galeri_t> galeriProblem =
                    Galeri::Xpetra::BuildProblem<SC, LO, GO,
                                        map_type, Tpetra::CrsMatrix<SC>, MV>
                                       (commandLineOptions.GaleriName2D, map, galeriList);
      A = galeriProblem->BuildMatrix();
#endif
      if (myRank == 0) {
        std::cout << "2D " << commandLineOptions.GaleriName2D << " matrix with nx=" << commandLineOptions.nx2 << endl;
      }
    }
      ...
      if (myRank == 0) {
        std::cout << " diagonal matrix with n = " << gblNumRows << endl;
      }
    }
    else {
      ...
      if (myRank == 0) {
        std::cout << " non-symm matrix with n = " << gblNumRows << endl;
      }
    }
  }
  A->describe (out);
  if (myRank == 0)
    std::cout << " >> Create matrix done << " << std::endl << std::endl;
  ...
  if (commandLineOptions.preconditioner != "") {
    ...
    if (commandLineOptions.preconditioner == "RELAXATION") {
      ...
      //
        ...
        PrecParams->set ("relaxation: outer sweeps", commandLineOptions.numOuterRelaxationSweeps);
        PrecParams->set ("relaxation: inner sparse-triangular solve", commandLineOptions.useInnerSptrsv);
        PrecParams->set ("relaxation: compact form", commandLineOptions.useCompactForm);
        PrecParams->set ("relaxation: inner damping factor", commandLineOptions.innerDampFactor);
      ...
      PrecParams->set ("relaxation: damping factor", commandLineOptions.dampFactor);
    }
    if (commandLineOptions.preconditioner == "SCHWARZ") {
      //int overlap = 0;
      //PrecParams->set ("schwarz: overlap level", overlap);
      if (commandLineOptions.subdomainSolver == "AMESOS2") {
        PrecParams->set ("relaxation: sweeps", commandLineOptions.numRelaxationSweeps);

        PrecParams->set ("subdomain solver name", "AMESOS2");
        if (commandLineOptions.subdomSolverType == "Superlu") {
          ...
          SuperluParams.set ("Equil", commandLineOptions.useEquil);
        }
        else if (commandLineOptions.subdomSolverType == "Superlu_dist") {
          ParameterList &SubdomainParams = PrecParams->sublist("subdomain solver parameters");
          SubdomainParams.set ("Amesos2 solver name", "Superlu_dist");

          ParameterList &Amesos2Params = SubdomainParams.sublist("Amesos2");
          ParameterList &SuperluParams = Amesos2Params.sublist("SuperLU_DIST");
          SuperluParams.set ("Equil", commandLineOptions.useEquil);
        }
      }
      else if (commandLineOptions.subdomainSolver == "RILUK") {
  ...
  if (SolverName == "TPETRA GMRES S-STEP") {
    params->set ("TSQR", commandLineOptions.tsqrType);
  }
  //
  ...
  if (SolverName == "TPETRA GMRES" ||
    ...
    if (commandLineOptions.sketchDom > 0) {
      params->set ("Sketch Domain", commandLineOptions.sketchDom);
    }
    if (myRank == 0) {
    ...
    params->set ("RandomQR",    commandLineOptions.useRandomQR);
    params->set ("RandomCGS2",  commandLineOptions.useRandomCGS2);
    params->set ("CholeskyQR",  commandLineOptions.useCholQR);
    params->set ("CholeskyQR2", commandLineOptions.useCholQR2);
    params->set ("TwoStage",    commandLineOptions.useTwoStage);
    params->set ("SplitCGS",    commandLineOptions.splitCGS);
    params->set ("SVQR",  commandLineOptions.useSVQR);
    params->set ("SVQR2", commandLineOptions.useSVQR2);
    params->set ("TSQR for initial step", commandLineOptions.useTSQR_1st_step);
  ...
  if(commandLineOptions.useStackedTimer) {
...
#define OLD_INTERFACE_
#ifdef OLD_INTERFACE_ // no need to register
